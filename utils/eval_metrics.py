...
def evaluate_wiki_recos(recos, output_path, gt_path, examples):
    ...
    if gt_path == "xlsum":
        article_recos_per_article = {art[0]: {art[0]+int(len(recos)): 1} for art in recos[:int(len(recos))]}
    else:
    ...
    reciprocals, mrr = calculate_mrr(recos_as_dict, article_recos_per_article, dataset_as_dict, names_to_id, titles=titles)
    ...
    return percentiles, mpr, reciprocals, mrr, hit_rates, hit_rate


def calculate_mpr(
    input_recommendations, article_article_gt, dataset, names_to_id, sample_size=-1, popular_titles=None, titles=[]
):
    with open('recos.pkl', 'wb') as handle:
        pickle.dump(input_recommendations, handle, protocol=pickle.HIGHEST_PROTOCOL)
    percentiles = []
    for reco_idx in tqdm(input_recommendations):
        wiki_title = titles[reco_idx]
        curr_gts, text = [], []
        recommendations = input_recommendations[reco_idx][1]
        if wiki_title not in article_article_gt:
            continue
        for gt_title in article_article_gt[wiki_title].keys():
            if isinstance(gt_title, str):
                lookup = gt_title.replace("&",
                                          "&amp;") if "amp;" not in gt_title and gt_title not in names_to_id else gt_title
            else:
                lookup = gt_title
            if lookup not in names_to_id:
                print(f"{lookup} not in names_to_id")
                continue
            recommended_idx_ls = np.where(recommendations.cpu() == names_to_id[lookup])[0]
            if recommended_idx_ls.shape[0] == 0:
                continue
            curr_gts.append(recommended_idx_ls[0])
            percentiles.extend((recommended_idx_ls[0] / len(recommendations),) * article_article_gt[wiki_title][gt_title])
        ...
        if len(curr_gts) > 0:
            print(
                "title: {}\n".format(wiki_title)
                + "\n".join(text)
                + "\ntopk: {}\n\n\n".format([titles[reco_i] for reco_i in recommendations[:10]])
            )
...
def calculate_mrr(
    ...
    """
        input_recommendations - list of [] per title the order of all titles recommended with it
    ...
    for reco_idx in tqdm(input_recommendations):
        ...
        if len(text) > 0:
            reciprocals.append(1 / (top))
            text.append(f"\n receprical: {reciprocals[-1]}")
            print(
                "title: {}\n".format(wiki_title)
                + "\n".join(text)
                + "\ntopk: {}\n\n\n".format([titles[reco_i] for reco_i in recommendations[:10]])
    ...
    reciprocals = reciprocals if reciprocals != [] else [0]
    print(f"Recepricle mean:{sum(reciprocals) / len(reciprocals)}")
    print(f"reciprocals \n {reciprocals}")
    return reciprocals, sum(reciprocals) / len(reciprocals)
